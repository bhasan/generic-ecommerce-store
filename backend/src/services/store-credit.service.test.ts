...
async function makeService() {
  const { StoreCreditService } = await import('./store-credit.service');
  return new StoreCreditService();
}

// Inspect the data passed to the most recent storeCreditTransaction.create call.
const lastCreate = () => prismaMock.storeCreditTransaction.create.mock.calls.at(-1)![0].data;
...
describe('StoreCreditService', () => {
  beforeEach(() => {
    ...
    prismaMock.storeCreditTransaction.create.mockResolvedValue({ id: 1 });
  ...
  describe('getUserCreditBalance', () => {
    it('returns the stored balance as a number', async () => {
      prismaMock.user.findUnique.mockResolvedValue({ storeCreditBalance: D(42.5) });
  ...
  describe('addCredit', () => {
    ...
    it('records an ADDED entry with balanceAfter and sets the new balance', async () => {
      prismaMock.user.findUnique.mockResolvedValue({ storeCreditBalance: D(10) });
      const service = await makeService();
      ...
      expect(lastUpdate().data.storeCreditBalance.toNumber()).toBe(35);
    });
  });
  ...
  describe('removeCredit', () => {
    ...
    it('refuses to overdraw the balance', async () => {
      prismaMock.user.findUnique.mockResolvedValue({ storeCreditBalance: D(5) });
      const service = await makeService();
    ...
    it('records a negative REMOVED entry and sets the decremented balance', async () => {
      prismaMock.user.findUnique.mockResolvedValue({ storeCreditBalance: D(50) });
      const service = await makeService();
      ...
      expect(lastUpdate().data.storeCreditBalance.toNumber()).toBe(30);
    });

    it('allows removing the exact remaining balance (boundary)', async () => {
      prismaMock.user.findUnique.mockResolvedValue({ storeCreditBalance: D(15) });
      const service = await makeService();
  ...
  describe('useCredit', () => {
    function makeTx() {
      return {
        ...
        storeCreditTransaction: { create: vi.fn().mockResolvedValue({ id: 9 }) },
      };
    }
    ...
    it('rejects when the balance is insufficient', async () => {
      const tx = makeTx();
      tx.user.findUnique.mockResolvedValue({ storeCreditBalance: D(3) });
      const service = await makeService();
      ...
      expect(tx.user.update).not.toHaveBeenCalled();
      expect(tx.storeCreditTransaction.create).not.toHaveBeenCalled();
    });

    it('decrements the balance and records a negative USED entry tied to the order', async () => {
      const tx = makeTx();
      tx.user.findUnique.mockResolvedValue({ storeCreditBalance: D(100) });
      const service = await makeService();

      await service.useCredit(7, 40, 55, tx as never);

      expect(tx.user.update.mock.calls[0][0].data.storeCreditBalance.toNumber()).toBe(60);
      const data = tx.storeCreditTransaction.create.mock.calls[0][0].data;
  ...
  describe('refundCredit', () => {
    it('records a positive REFUNDED entry tied to the order and increments the balance', async () => {
      prismaMock.user.findUnique.mockResolvedValue({ storeCreditBalance: D(40) });
      const service = await makeService();
  ...
  describe('reconcileBalance', () => {
    it('reports reconciled when the cached balance equals the ledger sum', async () => {
      prismaMock.user.findUnique.mockResolvedValue({ storeCreditBalance: D(30) });
      prismaMock.storeCreditTransaction.aggregate.mockResolvedValue({ _sum: { amount: D(30) } });
      const service = await makeService();
    ...
    it('flags a mismatch (e.g. a legacy balance with no ledger history)', async () => {
      prismaMock.user.findUnique.mockResolvedValue({ storeCreditBalance: D(445.89) });
      prismaMock.storeCreditTransaction.aggregate.mockResolvedValue({ _sum: { amount: null } });
      const service = await makeService();
